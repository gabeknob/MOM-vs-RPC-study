...
type MomClient struct {
	...
	pendingReqs map[string]chan *pb.SalesResponse
	mu          sync.Mutex
}
...
func (c *MomClient) PublishRequest(ctx context.Context, ch *amqp.Channel, req *pb.SalesRequest) (*pb.SalesResponse, error) {
	corrId := uuid.New().String()

	respChan := make(chan *pb.SalesResponse, 1)
	...
	case resp := <-respChan:
		return resp, nil
	case <-ctx.Done():
		return &pb.SalesResponse{}, ctx.Err()
	}
}
...
func main() {
	...
	mc := &MomClient{
		conn:        conn,
		pendingReqs: make(map[string]chan *pb.SalesResponse),
